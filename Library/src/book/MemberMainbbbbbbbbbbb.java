...
public class MemberMainbbbbbbbbbbb {
	private BufferedReader br;
	private String mem_id;//로그인한 아이디 저장
	private boolean flag;//로그인 여부
	private NoticeDAO notice;
	private MemberDAObbbb mem;
	private BookDAO dao;
	
	public MemberMainbbbbbbbbbbb() {
		try {
			br = new BufferedReader(new InputStreamReader(System.in));
			mem= new MemberDAObbbb ();
		...
		while(true) {
			...
			try {
				...
				if(no ==1) {
					...
					flag = mem.loginCheck(mem_id, mem_pw);
					if(flag) {
					...
					mem.insertInfo(id, passwd, name, cell, email);
				}else if (no==3) {
					dao.selectInfo();
					//도서 목록
