			...
			try {
				if(!dao.checkMemStop(mem_id) && no==5 ) {
					...
					do {
						...
						System.out.println();
						System.out.print("반납을 원하실 경우 대여번호, 뒤로가기를 원하실 경우 q(Q)를 입력해주세요\n > ");
						s = br.readLine();
						if(s.equals("Q") || s.equals("q")) {
							...
							try {
								flag = true;
								order_num = Integer.parseInt(s);
							} catch (NumberFormatException e) {

							}
						}
					} while (!dao.checkNowOrderNum(order_num, mem_id) && !s.equals("q") && !s.equals("Q"));
