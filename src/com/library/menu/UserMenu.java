...
public class UserMenu {
	...
	private void handleMenu(int choice) throws IOException {
		...
		switch (choice) {
		case 1:
			showUserAlerts();  // 사용자 알림 화면
			break;
		case 2:
			showBookList();  // 도서 목록 화면
			break;
		case 3:
			searchBooks();  // 도서 검색 화면
			break;
		case 4:
			showReviewPage();  // 리뷰 화면
			break;
		case 5:
			showRentReservePage();  // 대여/예약 화면
			break;
		case 6:
			showReturnPage();  // 반납 화면
			break;
		case 7:
			showOtherFunctions();  // 기타 기능 화면
			break;
		default:
	...
	private void showUserAlerts() throws IOException {
		...
		MemberService member = new MemberServiceImpl(memId);
	...
	private void showOtherFunctions() throws IOException {
		...
		while(true) {
			...
			try {
				...
				switch (choice) {
				case 1:
					WishBookService wishBook = new WishBookServiceImpl(memId);
					wishBook.manageWishBook(br);
					break;
				case 2:
					QnaService qna = new QnaServiceImpl(memId);
					qna.manageQNA(br);
					break;
				case 3:
					MemberService member = new MemberServiceImpl(new MemberDAOImpl(), memId);
